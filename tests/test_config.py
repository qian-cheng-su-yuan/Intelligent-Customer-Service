...
def test_settings_defaults_to_qwen_dashscope_provider(tmp_path):
    settings = Settings(dashscope_api_key="sk-test", database_path=tmp_path / "demo.db")
...
def test_settings_requires_api_key_for_live_llm(tmp_path):
    settings = Settings(dashscope_api_key="", llm_api_key="", database_path=tmp_path / "demo.db")
